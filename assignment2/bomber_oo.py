...
class Building():
    ...
    '''10 is the bomb width, and 22 is the bomb height'''
    def is_inside_for_bomb(self, point):
        if point.X < self.x - 10 or point.X > self.x + self.width or point.Y < CANVAS_HEIGHT-self.height - 22:
            return False
        return True
    ...
    def check_bomb(self):
        ...
        for building in self.buildings:
            if self.bomb.position.getY()>CANVAS_HEIGHT:
                self.bomb.explode()
            if building.is_inside_for_bomb(self.bomb.position):
                self.bomb.explode()
                building.shrink()
    ...
    def check_plane(self):
        ...
        for building in self.buildings:
            if (building.is_inside(plane_nose)
                ...
                self.game_over()
        if plane_body_bottom.getY() > CANVAS_HEIGHT-self.plane.height and plane_body_bottom.getX() < 20:
            self.plane_landed()
